...
#GLOBAL VARIABLES:
URL = "http://grafana.tangome.gbl:9200/grafana-dash/dashboard/"         #ElasticSearch API
DASHBOARD_URL = "http://grafana.tangome.gbl/grafana/#/dashboard/db/"    #Part of the path to the new graph.
...
def createDashboard(metric_name=None):

    #TODO: think about it.
    dashboard_name = str(metric_name)
    ...
    #Modify title in the dashboard dictionary.
    jsonDashboard_dict["title"] = metric_name

    #Modify title in second place in the dashboard dictionary.
    jsonDashboard_dict["rows"][0]["panels"][0]["title"] = metric_name

    #Modify title in the json_main_dict.
    json_main_dict["title"] = "_TEMP_" + str(metric_name)
    ...
    #TODO: Why?
    # new_dashboard_url = urllib.parse.urljoin(DASHBOARD_URL, dashboard_name)
    new_dashboard_url = DASHBOARD_URL + dashboard_name

    return (r.status_code, new_dashboard_url)

def main():
    new_dashboard_metric = "memcache.us0101mmb001.memcached.account_to_email.memcached_ops.hits"
    createDashboard(new_dashboard_metric)
